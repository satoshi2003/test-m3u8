...
def fetch_and_parse_m3u(url):
    try:
        ...
        # Truyền URL vào hàm parse để đánh dấu nguồn
        return parse_m3u(content, source_url=url)
    except Exception as e:
        print(f"Lỗi khi xử lý {url}: {str(e)[:50]}")
        return []

def parse_m3u(content, source_url="Unknown"):
    channels = []
    current_ch = {}
    extra_lines = []
    lines = content.split('\n')
    for line in lines:
        ...
        if line.startswith('#EXTINF'):
            ...
            if current_ch and 'name' in current_ch:
                current_ch['url'] = line
                current_ch['source'] = source_url  # Lưu lại link nguồn của kênh này
                if extra_lines:
                    current_ch['extra'] = extra_lines
                channels.append(current_ch)
                current_ch = {}
